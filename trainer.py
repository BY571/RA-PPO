...
class Trainer():
    def __init__(self, config):
        ...
        self.agent = PPOAgent(observation_space,
                           self.action_space,
                           config)

        self.buffer = PPOBuffer(buffer_size=self.update_frequency,
    ...
    def train(self, ):
        with wandb.init(project=self.config.project,
            ...
            wandb.log({"episode": total_episodes, "test_return": test_return}, step=current_step)

            while current_step < max_interactions:
                ...
                for t in range(self.max_ep_len):
                    ...
                    # Collection step
                    next_state, reward, done, _ = self.train_env.step(action.cpu().numpy())
                    ...
                    if done.any():
                        ...
                        train_log_info = {"episode": total_episodes,
                                  "train_return": total_reward.mean()}
                        wandb.log(train_log_info, step=current_step)

                        print("Episode: {} -- train_return: {}".format(total_episodes, total_reward.mean()))
                        break
