...
class PPOBuffer:
    ...
    def __init__(self, buffer_size, num_envs, state_dim, action_space, device):
        ...
        # Preallocate memory
        self.states = torch.zeros((buffer_size, num_envs, state_dim), dtype=torch.float32).to(device)
        self.actions = torch.zeros((buffer_size, num_envs, self.action_dim), dtype=self.action_dtype).to(device)
        self.log_probs = torch.zeros((buffer_size, num_envs), dtype=torch.float32).to(device)
        self.rewards = torch.zeros((buffer_size, num_envs), dtype=torch.float32).to(device)
        self.next_states = torch.zeros((buffer_size, num_envs, state_dim), dtype=torch.float32).to(device)
    ...
    def add(self, states, actions, log_probs, rewards, next_states, dones):
        ...
        self.states[self.idx] = torch.tensor(states, dtype=torch.float32).to(self.device)
        self.actions[self.idx] = torch.tensor(actions, dtype=self.action_dtype).to(self.device)[:, None]
        self.log_probs[self.idx] = torch.tensor(log_probs, dtype=torch.float32).to(self.device)
        self.rewards[self.idx] = torch.tensor(rewards, dtype=torch.float32).to(self.device)
        self.next_states[self.idx] = torch.tensor(next_states, dtype=torch.float32).to(self.device)
