...
from networks import ContinuousActor, Critic, DCritic, DiscreteActor
...
class PPOAgent:
    def __init__(self, observation_space, action_space, config):
        ...
        if isinstance(action_space, Box):
            self.actor = ContinuousActor(
                observation_space, action_space, config.hidden_size
            ).to(self.device)
        ...
        self.critic = DCritic(observation_space, action_space, config.hidden_size).to(
            self.device
        )
    ...
    def update(self, states, actions, log_probs_old, rewards, next_states, dones):
        ...
        with torch.no_grad():
            ...
            next_quantiles, _ = self.critic(
                next_states.flatten(0, 1), n_taus=64, lb=0, ub=eta
            )
        ...
        quant_advantage = self.vmap_gae(
            rewards[..., None].expand(-1, self.num_envs, 64, 1),
            quantiles,
            next_quantiles,
            dones[..., None].expand(-1, self.num_envs, 64, 1),
        )
        ...
        targets_values = target_value.reshape(-1, 64, target_value.shape[-1])

        # PPO update loop
        bs = states.shape[0]
        num_updates = bs // self.minibatch_size
        assert num_updates > 0, "Batch size must be greater than minibatch size"
        ...
        for _ in range(num_updates):
            for start in range(0, states.shape[0], self.minibatch_size):
                ...
                loss_info = self._update(
                    states[sample_indices],
                    actions[sample_indices],
                    log_probs_old[sample_indices],
                    advantages[sample_indices],
                    targets_values[sample_indices],
                )
    ...
    def _update(self, states, actions, log_probs_old, advantages, target_value):
        ...
        element_wise_huber_loss = self.calculate_huber_loss(
            td_errors
        )  # (batch_size, 64, 64)

        quantile_huber_loss = self.calculate_quantile_huber_loss(
            element_wise_huber_loss, td_errors, taus
        )  # (batch_size, 64, 64)
